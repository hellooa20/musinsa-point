...
고객별 무료 포인트의 일반 적립, 관리자 수기 적립, 적립 취소, 사용, 전체·부분 사용 취소를 제공한다.<br/>
...
| 만료 포인트 복원 | 사용 취소 시 원적립이 만료된 경우 유효기간 7일의 신규 적립을 생성<br/>해당 재적립에는 1회 적립 한도를 적용하지 않음                                                          |
...
| 항목 | 관리 위치 | 현재 값 |
|---|---|---:|
| 1회 최대 적립 금액 | `application.yml`의 `point.max-accrual-amount` | 100,000 |
| 기본 적립 유효기간 | `application.yml`의 `point.default-validity-days` | 365일 |
| 만료 포인트 재적립 유효기간 | `application.yml`의 `point.expired-refund-validity-days` | 7일 |
| 고객별 보유 한도 | `PUT /api/v1/point-policies/{customerId}` | 고객별 설정 |
...
- 참조 대상의 존재 여부는 데이터베이스의 외래 키 제약조건으로 보장한다.
- 동일 고객, 거래 유형, 원장과 상세 금액 관계 등의 비즈니스 정합성은 애플리케이션에서 검증한다.

### 동시성
- 같은 고객의 변경은 `PESSIMISTIC_WRITE` 비관적 락을 사용해 하나씩 처리한다.
- 고객 정책 최초 생성은 잠글 행이 없으므로 H2 `MERGE`를 사용해 동시에 요청되어도 한 행만 생성한다.
- 다른 고객의 요청은 서로 막지 않는다.
- H2 락 대기 시간은 2초로 설정하며, 타임아웃은 `503 LOCK_TIMEOUT`으로 응답한다.

### 멱등성
- `requestId`는 락 획득 전과 후에 검사한다.
- `point_ledger.request_id`의 `UNIQUE` 제약조건으로 동시 중복 요청을 데이터베이스에서도 최종 방어한다.
...
> 포인트 적립부터 사용 및 사용 취소까지의 흐름을 나타낸 시나리오이며, 사용 취소 시 원적립이 만료된 경우도 포함한다.<br/>
> 상세 source → target은 원장 상세에 저장되는 포인트의 차감·복원 관계를 의미한다.
...
> 각 API는 [point-api.http](point-api.http) 파일을 통해 테스트가 가능하다.
...
- 동시성: 같은 고객의 동시 요청에서도 초과 차감이나 중복 처리가 발생하지 않는지 검증

> 과제에서 제공한 예시 시나리오를 기준으로 포인트 적립, 사용, 부분 사용 취소 및 만료 포인트 재적립 흐름을 [point-assignment-example.http](point-assignment-example.http)에 정리했다.
> <br/>애플리케이션 실행 후 해당 파일의 요청을 위에서부터 순서대로 실행하면 전체 흐름을 확인할 수 있다.

## 설계 고려사항 및 개선 방향

### 1. 사용 취소 순서

- **현재 방식**: 과제 예시를 재현하기 위해 원사용 상세 순서대로 복원하는 FIFO 방식을 적용한다.
- **개선 방향**: 고객에게 유리한 포인트부터 복원해야 하는 정책이 추가되면 LIFO 등 다른 복원 순서를 검토한다.

### 2. 포인트 만료 처리

- **현재 방식**: 별도의 만료 스케줄러나 만료 원장을 생성하지 않고, 조회 시각을 기준으로 만료 여부를 판단한다.
- **개선 방향**: 고객에게 명시적인 만료 이력을 제공해야 한다면 만료 대상 처리 스케줄러와 만료 원장을 추가한다.

### 3. 대용량 원장 관리

- **현재 방식**: H2를 사용하므로 테이블 파티셔닝을 적용하지 않는다.
- **개선 방향**: 운영 환경에서 거래 데이터가 증가하면 거래일자 기준의 월 단위 파티셔닝을 검토한다.
- **추가 고려사항**: 파티셔닝 적용 시 거래 식별자의 유일성 보장 방식도 함께 검토해야 한다.

### 4. 고객 포인트 잔액 조회

- **현재 방식**: 고객별 잔액을 별도로 저장하지 않고, 사용 가능한 적립 원장의 `remainingAmount`를 합산하여 조회한다.
- **한계**: 고객이 보유한 적립 원장이 많아지면 잔액 조회 비용이 증가할 수 있다.
- **개선 방향**: 조회 트래픽이 증가하면 고객별 잔액 테이블을 별도로 관리하고, 원장과 잔액의 정합성을 검증하는 절차를 추가한다.

### 5. 포인트 사용 및 사용 취소 성능

- **현재 방식**: 사용 가능한 적립 원장을 순서대로 조회한 뒤, 적립 건별로 잔액을 차감하거나 복원하고 원장 상세를 생성한다.
- **한계**: 한 고객이 보유한 적립 원장이 많으면 UPDATE와 INSERT 실행 횟수가 증가하여 처리 시간이 길어질 수 있다.
- **개선 방향**:
  * 쿼리에서 차감·복원 대상과 금액을 계산한다.
  * 적립 원장 잔액은 일괄 UPDATE한다.
  * 원장 상세는 배치 INSERT로 저장한다.
  * 이를 통해 데이터베이스 접근 횟수를 줄인다.

### 6. 인증 및 권한 관리

- **현재 방식**: 인증과 권한 관리는 과제 구현 범위에서 제외한다.
- **운영 환경 고려사항**:
  * 로그인한 고객이 자신의 포인트만 조회·변경할 수 있도록 소유권을 검증한다.
  * 관리자 수기 적립 API에는 관리자 권한을 적용한다.
  * 요청의 `customerId`나 `pointKey`만으로 사용자를 신뢰하지 않는다.

### 7. API 요청 위·변조 방지

- **현재 방식**: `requestId`로 동일 요청의 중복 처리를 방지하지만, 요청 발신자와 전송 중 데이터 무결성을 검증하는 인증·서명 기능은 과제 범위에서 제외했다.
- **한계**: 단순 해시값을 요청에 포함하는 방식은 공격자가 요청 값과 해시값을 함께 변경할 수 있어 위·변조 방지 수단이 되지 않는다. 또한 `requestId`는 멱등성 식별자이므로 인증·인가를 대신할 수 없다.
- **개선 방향**:
  * 운영 환경에서는 HTTPS를 강제하여 전송 중 요청 탈취와 변조를 방지한다.
  * 서버 간 요청에는 HTTP Method, 경로, 요청 본문, 요청 시각, `requestId`를 정규화한 뒤 공유 비밀키로 생성한 `HMAC-SHA256` 서명을 헤더에 포함한다.
  * 서버는 서명을 상수 시간으로 비교하고, 허용 시간 범위와 이미 처리된 `requestId`를 함께 검사하여 위·변조와 재전송 공격을 방지한다.
  * 공유 비밀키는 애플리케이션 코드에 저장하지 않고 별도의 비밀 관리 서비스에서 관리하고 주기적으로 교체한다.
